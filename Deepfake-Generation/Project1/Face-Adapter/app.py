...
snapshot_download(repo_id="FaceAdapter/FaceAdapter", local_dir="/netscratch/amkhan/FaceAdapter/checkpoints")
...
controlnet = ControlNetModel.from_pretrained('/netscratch/amkhan/FaceAdapter/checkpoints/controlnet', torch_dtype=weight_dtype).to(device)

pipe = StableDiffusionFaceAdapterPipeline.from_pretrained(
    '/netscratch/amkhan/pretrained_models/stable-diffusion-v1-5', controlnet=controlnet, torch_dtype=weight_dtype, cache_dir='/netscratch/amkhan/FaceAdapter/hub' if use_cache else None, local_files_only=use_cache, requires_safety_checker=False
).to(device)
# pretrained unet
pretrained_unet_path = '/netscratch/amkhan/FaceAdapter/checkpoints/pretrained_unet'
...
vae_ft_mse = AutoencoderKL.from_pretrained("stabilityai/sd-vae-ft-mse", cache_dir='/netscratch/amkhan/FaceAdapter/hub' if use_cache else None, torch_dtype=weight_dtype, local_files_only=use_cache).to(device)
pipe.vae = vae_ft_mse

net_d3dfr = model_resnet_d3dfr.getd3dfr_res50('/netscratch/amkhan/FaceAdapter/checkpoints/third_party/d3dfr_res50_nofc.pth').eval().to(device)
bfm_facemodel = bfm.BFM(focal=1015*256/224, image_size=256, bfm_model_path='/netscratch/amkhan/FaceAdapter/checkpoints/third_party/BFM_model_front.mat').to(device)

net_arcface = model_insightface_backbone.getarcface('/netscratch/amkhan/FaceAdapter/checkpoints/third_party/insightface_glint360k.pth').to(device)
clip_image_processor = CLIPImageProcessor()

# "laion/CLIP-ViT-H-14-laion2B-s32B-b79K"
net_vision_encoder = CLIPVisionModel.from_pretrained('/netscratch/amkhan/FaceAdapter/checkpoints/vision_encoder').to(device)
...
net_image2token.load_state_dict(torch.load('/netscratch/amkhan/FaceAdapter/checkpoints/net_image2token.pth'))
    
net_id2token = ID2Token(id_dim=512, text_hidden_size=768, max_length=77, num_layers=3).to(device)
net_id2token.load_state_dict(torch.load('/netscratch/amkhan/FaceAdapter/checkpoints/net_id2token.pth'))

net_seg_res18 = model_seg_unet.UNet().eval().to(device)
net_seg_res18.load_state_dict(torch.load('/netscratch/amkhan/FaceAdapter/checkpoints/net_seg_res18.pth'))
app = FaceAnalysis(name='antelopev2', root='/netscratch/amkhan/FaceAdapter/checkpoints/third_party', providers=['CUDAExecutionProvider', 'CPUExecutionProvider'])
...
def generate_image(
    ...
    ##########    face swapping   ########## 
    accurate_mask_path = '/netscratch/amkhan/FaceAdapter/checkpoints/mask/' + drive_img_path.split('.')[0] + '.png'
    if os.path.isfile(accurate_mask_path):
        ...
        #  (17, 'hair'), (18, 'hat')
        net_seg = model_parsing.get_face_parsing('/netscratch/amkhan/FaceAdapter/checkpoints/third_party/79999_iter.pth').eval().to(device)        
